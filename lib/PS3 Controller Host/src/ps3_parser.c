...
enum ps3_packet_index {
    ps3_packet_index_buttons_raw = 14,
...
enum ps3_button_mask {
    ...
    ps3_button_mask_up       = 0xffffff7f,
    ps3_button_mask_right    = 0xfffffd7f,
    ps3_button_mask_down     = 0xfffffd7f,
    ps3_button_mask_left     = 0xfffff97f,
...
for (int i = 15; i < 19; i++) { // Replace PACKET_SIZE with the actual size of the packet
    if (packet[i] < 0x10) {  // For single digit hex numbers, add a leading zero for better readability
       
    }
    printf("%02X ", packet[i]); // %02X ensures two-digit hex format
}
printf("\n");
*/
...
ps3_event_t ps3_parse_event( ps3_t prev, ps3_t cur )
{
    ...
    /* Analog events */
    ps3_event.analog_changed.stick.lx        = cur.analog.stick.lx;
    ps3_event.analog_changed.stick.ly        = cur.analog.stick.ly;
    ps3_event.analog_changed.stick.rx        = cur.analog.stick.rx;
    ps3_event.analog_changed.stick.ry        = cur.analog.stick.ry;
...
ps3_button_t ps3_parse_packet_buttons( uint8_t *packet )
{
  ...
  // printf("button: 0x%X\n", ps3_buttons_raw);
 //  uint16_t ps3_buttons_raw = *((uint16_t*)&packet[15]);
 //  uint16_t ps3_buttons_raw2 = *((uint16_t*)&packet[17]);
 
    // printf("buttons2 %02x\n",ps3_buttons_raw);
    uint8_t ps3_buttons_cros = (*((uint8_t*)&packet[15]));
    uint8_t ps3_buttons_figure = (*((uint8_t*)&packet[16]));
   // printf("buttons2 %02x\n",ps3_buttons_figure);

    ps3_button.up    =   (ps3_buttons_cros == 0x00) ? true : false;
    ps3_button.right =   (ps3_buttons_cros == 0x02) ? true : false;
    ps3_button.down  =   (ps3_buttons_cros == 0x04) ? true : false;
    ps3_button.left  =   (ps3_buttons_cros == 0x06) ? true : false;

    ps3_button.triangle = (ps3_buttons_figure == 0x10) ? true : false;
    ps3_button.circle   = (ps3_buttons_figure == 0x02) ? true : false;
    ps3_button.cross    = (ps3_buttons_figure == 0x01) ? true : false;
    ps3_button.square  =  (ps3_buttons_figure == 0x08) ? true : false;

      
 // Define the button states
 /*
    bool button_up = (ps3_buttons_cros == 0x00) ? true : false;
    bool button_right = (ps3_buttons_cros == 0x02) ? true : false;
    bool button_down = (ps3_buttons_cros == 0x04) ? true : false;
    bool button_left = (ps3_buttons_cros == 0x06) ? true : false;

   

    bool button_triangle =(ps3_buttons_figure == 0x10) ? true : false;
    bool button_circle  = (ps3_buttons_figure == 0x02) ? true : false;
    bool button_cross =   (ps3_buttons_figure == 0x01) ? true : false;
    bool button_square  = (ps3_buttons_figure == 0x08) ? true : false;
    ...
    if (button_left) {
     ...
     if (button_triangle) {
        printf("Button: triangle\n");
    }
    if (button_circle) {
        printf("Button: circle\n");
    }
    if (button_cross) {
        printf("Button: Cross\n");
    }
